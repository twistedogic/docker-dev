#!/bin/bash
location=$(pwd)
CID=$(docker run -h master --name mesos -m=104857600 -d -p 5050:5050 -p 172.17.42.1:53:8600 -p 172.17.42.1:53:8600/udp --dns=172.17.42.1 --dns=8.8.8.8 -p 2181:2181 -p 8500:8500 twistedogic/mesos-master)
...
S1=$(docker run -h slave1 --name mesos-slave1 -p 5051:5051 --dns=172.17.42.1 --dns=8.8.8.8 -d -v ${location}/slave_conf:/etc/supervisor/conf.d/ -m=104857600 twistedogic/mesos-slave)
...
#S3=$(docker run -d -p 8083:8080 -v ${location}/storm_conf/:/opt/storm/conf/ twistedogic/storm)
docker ps -a
